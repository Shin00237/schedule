 ...
 *
 * @param minOverlapEmployees Nombre minimum d'employés qui doivent être présents dans les deux
 *     shifts chevauchants
 *     <p>Exemple d'usage: scheduler.withConstraint(new ShiftOverlapConstraint(1,
 *     ConstraintNature.HARD, null));
 *     <p>Complexité: O(shifts² × employees)
 */
      ...
      for (int s2 = s1 + 1; s2 < shiftCount; s2++) {
        ...
        if (shiftsOverlap(shift1, shift2)) {
          ...
          for (int e = 0; e < employeeCount; e++) {
            IntVar overlapVar =
                context.getModel().newBoolVar("overlap_e" + e + "_s" + s1 + "_s" + s2);
      ...
      for (int s2 = s1 + 1; s2 < shiftCount; s2++) {
        ...
        if (shiftsOverlap(shift1, shift2)) {
          ...
          IntVar violationVar =
              context
                  .getModel()
                  .newIntVar(0, employeeCount, "overlap_violation_s" + s1 + "_s" + s2);
          ...
          context
              .getModel()
              .addMaxEquality(violationVar, new LinearExpr[] {LinearExpr.constant(0), deficit});
