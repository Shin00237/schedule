 ...
 * 
 * Cette classe record encapsule toutes les informations nécessaires pour
 * créer et configurer une contrainte :
 * - Le type de contrainte à appliquer
 * - Sa nature (HARD ou SOFT)
 * - Sa priorité d'application
 * - Ses paramètres spécifiques
 * 
 * Cette approche permet une configuration flexible depuis une interface
 * graphique ou des fichiers de configuration.
 * 
...
public record ConstraintConfig(
     ...
     */
    public static ConstraintConfig of(ConstraintType type, ConstraintNature nature, ConstraintPriority priority) {
    ...
    public ConstraintConfig {
        ...
        if (parameters == null) {
            throw new IllegalArgumentException("Les paramètres ne peuvent pas être null (utiliser Map.of() pour une map vide)");
        }
