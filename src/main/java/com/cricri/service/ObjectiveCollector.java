 ...
 *
 * <p>Cette classe permet aux contraintes souples de contribuer à un objectif global unifié sans
 * avoir besoin d'un état global mutable dans le SchedulingContext.
 *
 * <p>Fonctionnement : 1. Chaque contrainte SOFT ajoute ses termes via addTerm() 2. À la fin,
 * build() construit l'expression LinearExpr complète 3. Un seul model.maximize() est appelé avec
 * l'objectif unifié
 *
 * <p>Thread-safe : Les méthodes addTerm() sont synchronisées pour permettre un traitement parallèle
 * des contraintes.
 */
public class ObjectiveCollector {
   ...
   *
   * <p>Cette méthode est thread-safe et peut être appelée par plusieurs contraintes en parallèle.
   *
   ...
   *
   * <p>Cette méthode peut être appelée plusieurs fois et retournera toujours la même expression
   * basée sur les termes actuels.
   *
