 ...
 *
 * Cette contrainte assure que chaque shift a le nombre minimum et maximum
 * d'employés requis selon sa configuration.
 */
class MinimumCoverageConstraintTest extends ConstraintTestBase {
  ...
  void testCoverageWithRangeRequirements() {
    ...
    List<Shift> rangeShifts = List.of(
        new Shift("Flexible-1", week1.getDay(0), TestDataFactory.NORMAL_SHIFT, 1, 2), // 1-2 employés
        new Shift("Flexible-2", week1.getDay(1), TestDataFactory.NORMAL_SHIFT, 1, 3)  // 1-3 employés
    );
  ...
  void testInfeasibleScenario() {
    ...
    List<Shift> impossibleShifts = List.of(
        new Shift("Impossible", week1.getDay(0), TestDataFactory.NORMAL_SHIFT, 2, 2)
    );

    SchedulingContext impossibleContext = TestDataFactory.createContext(oneEmployee, impossibleShifts);
    testConstraintMakesScenarioInfeasible(constraint, impossibleContext,
        "1 employé pour un shift qui demande 2 employés minimum");
  }
  ...
  void testMultipleShiftsWithDifferentRequirements() {
    ...
    List<Shift> variedShifts = List.of(
  ...
  void testConstraintWithZeroMaxEmployees() {
    ...
    List<Shift> optionalShifts = List.of(
    ...
    for (int e = 0; e < employees.size(); e++) {
      assertEquals(0, solver.value(optionalContext.getAssignments()[e][0]),
