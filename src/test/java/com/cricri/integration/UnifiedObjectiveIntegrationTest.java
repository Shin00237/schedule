 ...
 *
 * <p>Valide que toutes les contraintes SOFT contribuent correctement à l'objectif global
 * et que le système fonctionne end-to-end.
 */
class UnifiedObjectiveIntegrationTest {
    ...
    void testSingleSoftConstraintIntegration() {
        ...
        List<ConstraintConfig> configs = Arrays.asList(
    ...
    void testConstraintPriorityOrdering() {
        ...
        // Then - L'ordre d'application devrait être respecté (FUNDAMENTAL -> SAFETY -> NORMAL -> OPTIMIZATION)
        assertDoesNotThrow(() -> scheduler.buildModel());
        assertSolutionExists(scheduler.getModel());
    }
    ...
    void testRegressionMaximizeWorkingHoursBehavior() {
        ...
        List<ConstraintConfig> configs = Arrays.asList(
            ...
            ConstraintConfig.of(
                ConstraintType.MAXIMIZE_WORKING_HOURS,
                ConstraintNature.SOFT,
                "weekdayMultiplier", 2 // Comme dans l'ancien système
            )
        );
    ...
    void testHighConflictScenario() {
        ...
        List<ConstraintConfig> configs = Arrays.asList(
            ...
            ConstraintConfig.of(
                ConstraintType.MAXIMIZE_WORKING_HOURS,
                ConstraintNature.SOFT,
                "weekdayMultiplier", 3 // Pousse vers plus d'heures
            ),
            ConstraintConfig.of(
                ConstraintType.MAX_HOURS_PER_WEEK,
                ConstraintNature.SOFT,
                "maxHoursPerWeek", 25 * 60 // Limite basse
            ),
            ConstraintConfig.of(
                ConstraintType.MINIMUM_REST_DAYS,
                ConstraintNature.SOFT,
                "minRestDaysPerWeek", 3 // Beaucoup de repos
            ),
            ConstraintConfig.of(
                ConstraintType.MINIMUM_REST,
                ConstraintNature.SOFT,
                "minRestHours", 12 // Repos long entre shifts
            )
        );
